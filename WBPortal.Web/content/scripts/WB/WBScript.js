...
var $ae = new function () {
    ...
    this.fullscreen = function (o) {
        ...
        {
            var opt = { height: $(window).height() - 50, width: $(window).width() - 50 };
            $(o).dialog(opt).trigger('dialogresize');
        });
    ...
    this.popup = function (o, w, h, title, modal, pos, res, btns, fulls) {
        ...
        $("#" + o).dialog({
            ...
            buttons: btns,
            autoOpen: false,
            close: function (e, ui) { $("#" + o).find('*').remove(); if (isReloadOnClose) { window.top.location.href = window.top.location.href; } }
        });
