...
(function ($) {
    ...
    //    $.getScript("/js/fileuploadjs/jquery.form.js");
    //}
    var feature = {};
    ...
    $.fn.uploadFile = function (options) {
        ...
        var uploadLabel = $('<div> Add Files </div>');
        ...
        function setDragDropHandlers(obj, s, ddObj) {
            ...
            ddObj.on('drop', function (e) {
                ...
                if (!s.multiple && files.length > 1) {
                    if (s.showError) $("<div style='color:red;font-size: 11px; margin-top: 5px;'>" + s.multiDragErrorStr + "</div>").appendTo(obj.errorLog);
                    return;
                }
        ...
        function serializeAndUploadFiles(s, obj, files) {
            for (var i = 0; i < files.length; i++) {
                if (!isFileTypeAllowed(obj, s, files[i].name)) {
                    if (s.showError) $("<div style='color:red;font-size: 11px; margin-top: 5px;'><b>" + files[i].name + "</b> " + s.extErrorStr + s.allowedTypes.toString().replace(/,/g,' ,') + "</div>").appendTo(obj.errorLog);
                    continue;
                }
                
                if (s.maxFileSize != -1 && files[i].size > s.maxFileSize) {
                    if (s.showError) $("<div style='color:red;font-size: 11px; margin-top: 5px;'><b>" + files[i].name + "</b> " + s.sizeErrorStr + getSizeStr(s.maxFileSize) + "</div>").appendTo(obj.errorLog);
                    continue;
                }
                var ts = s;
                var fd = new FormData();
